...
gulp.task('compile-hbs', () => {
  ...
  const partials = gulp
    .src([`${dir.source.public}/partials/**/*.hbs`])
    .pipe(handlebars())
    .pipe(
      wrap(
  ...
  const templates = gulp
    .src(`${dir.source.public}/templates/**/*.hbs`)
    .pipe(handlebars())
  ...
  return merge(partials, templates).pipe(concat('templates.js')).pipe(gulp.dest(dir.build.js));
});

// --- COMPILE CSS
gulp.task('compile-css', () => gulp
  .src(`${dir.source.css}/*.css`)
...
// --- COPY JS HBS
gulp.task('copy-js-hbs', () => gulp.src(`${root.npm}/handlebars/dist/handlebars.runtime.js`).pipe(gulp.dest(dir.build.js)));

// --- COPY HTML
gulp.task('copy-html', () => gulp.src(`${dir.source.public}/pages/*.html`).pipe(gulp.dest(dir.build.base)));
